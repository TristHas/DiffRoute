...
## Iteration 18 - Dense-key COO to block-sparse conversion

- Target: `BlockSparseKernel.from_coo` forward sparse conversion.
- Hypothesis: The generic `torch.unique(block_coords, dim=0, return_inverse=True)` path sorts 1.65M block-coordinate rows every forward. Since the block grid is only about 973 by 973 for RAPID, a per-call dense block-key map should recompute topology from the current graph while avoiding the general 2D unique path.
- Change: Added a CUDA fast path in `BlockSparseKernel.from_coo` that builds row-major block keys, marks present blocks in a dense boolean map, materializes block indices from `nonzero`, maps each COO row to its block id, and scatters current values into fresh `block_values`. The existing unique-based implementation remains the fallback.
- Correctness: A direct COO round-trip comparison against the old unique path gave `same_coords=True`, `max_abs_vals=0.0`, and identical `27088` block count. `python benchmarks/rapid_io_benchmark.py --correctness --correct-time-steps 16 --atol 2e-2` passed with `conv_forward_max_abs_vs_torch=6.602203939110041e-08`, `conv_backward_dx_max_abs_vs_torch=0.0024261474609375`, `conv_backward_dw_max_abs_vs_torch=5.21540641784668e-08`, and finite parameter gradients.
- Runtime: After removing cached/no-grad paths, the cleaned T=500 baseline was `full.forward median_ms=19.753520011901855`, `substep.blockize.forward median_ms=1.8762720227241516`. With dense-key conversion, `python benchmarks/rapid_io_benchmark.py --time-steps 500 --trials 10 --warmup 3 --backward-trials 1 --backward-warmup 0` reported `full.forward median_ms=19.323951721191406`, `substep.blockize.forward median_ms=1.4591360092163086`, and `substep.convolution.forward median_ms=3.5251519680023193`.
- Baseline comparison: This reduces blockization latency by `22.2%` and T=500 full-forward median by `2.2%` without caching or changing router-level execution paths.
- Notes: This recomputes graph-dependent sparse conversion state every call. It is a localized data-structure fast path, not a graph or value cache.

## Iteration 19 - Index-copy block value packing

- Target: CUDA dense-key `BlockSparseKernel.from_coo` block-value scatter.
- Hypothesis: Dense-key conversion maps each COO row to a unique row in flattened block-value storage, so `index_copy_` should avoid overhead from the more general `index_put` path while preserving the same fresh per-call block values.
- Change: Replaced `block_values.index_put((linear_indices,), values)` with in-place `block_values.index_copy_(0, linear_indices.contiguous(), values)` in the CUDA dense-key conversion path. This keeps the same block indices, value layout, and backward metadata construction.
- Correctness: `python benchmarks/rapid_io_benchmark.py --correctness --correct-time-steps 16 --atol 2e-2 --conv-impl auto` passed with `conv_forward_max_abs_vs_torch=6.599293556064367e-08`, `conv_backward_dx_max_abs_vs_torch=0.0024261474609375`, `conv_backward_dw_max_abs_vs_torch=5.21540641784668e-08`, and finite parameter gradients.
- Runtime: A direct microbenchmark on RAPID-305 sparse conversion measured current dense-key `from_coo` at about `1.4387 ms` median and the `index_copy_` variant at about `1.0254 ms` median with identical block indices, block values, column order, and offsets. The standard profile command `python benchmarks/rapid_io_benchmark.py --time-steps 500 --trials 10 --warmup 3 --backward-trials 1 --backward-warmup 0 --conv-impl auto` reported `full.forward median_ms=18.930496215820312`, `substep.blockize.forward median_ms=1.0644960403442383`, and `substep.convolution.forward median_ms=3.5361440181732178`.
- Baseline comparison: Compared with iteration 18, blockization median improves from `1.4591360092163086 ms` to `1.0644960403442383 ms` (`1.37x`, `27.0%` latency reduction). T=500 full-forward median improves from `19.323951721191406 ms` to `18.930496215820312 ms` (`2.0%`).
- Notes: This is a low-level sparse-packing primitive change. It does not cache routing kernels, graph topology conversions, sampled values, or any torch-level router/aggregator artifact.

## Iteration 20 - Dense-map backward metadata

- Target: CUDA dense-key `BlockSparseKernel.from_coo` construction of `block_col_order` and `block_col_offsets`.
- Hypothesis: After the dense presence map is built, column CSR metadata can be derived by scanning that map in column-major order instead of entering the generic constructor path, which sorts block indices and recomputes offsets.
- Change: The dense-key path now computes column counts, offsets, and column-major block order directly from the dense `present` map and passes them into `BlockSparseKernel`, avoiding the generic `_make_col_order` / `_make_col_offsets` work for this path.
- Correctness: `python benchmarks/rapid_io_benchmark.py --correctness --correct-time-steps 16 --atol 2e-2 --conv-impl auto` passed with `conv_forward_max_abs_vs_torch=6.59783836454153e-08`, `conv_backward_dx_max_abs_vs_torch=0.0024261474609375`, `conv_backward_dw_max_abs_vs_torch=5.21540641784668e-08`, and finite parameter gradients.
...
- Full-GEOGloWS validation: Building all 125 VPUs with `--cache /tmp/full_geoglows_graph_cache.pt --time-steps 29220 --warmup-time-steps 512 --trials 1` wrote the cache after `setup_seconds=274.29425988695584` and measured `random_runoff_plus_staged_routing=15.759096010995563 s`, `random_generation_only=0.4733391680056229 s`. Reusing that cache measured `setup_seconds=0.3659620829857886`, `random_runoff_plus_staged_routing=15.02335176197812 s`, `random_generation_only=0.47314729599747807 s`.
