...
from ..utils import get_node_idxs, annotate_downstream_path_stats
...
def test_init_pre_indices_new(g, node_idxs, include_self):
    from .index_precompute import init_pre_indices as init_pre_indices_new
    a,b,c = init_pre_indices_new(g, node_idxs, include_self=include_self)
    a1,b1,c1 = init_pre_indices(g, node_idxs, include_self=include_self)
    assert (a1==a).all().item() and (b1==b).all().item() and (c1==c).all().item()
